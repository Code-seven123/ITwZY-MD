...
function json(){
  return JSON.parse(fs.readFileSync(join(__dirname, '../premium.json')))
}

const dataNew = async () => {
  return new Promise((resolve) => {
    resolve(json())
    fs.watch(join(__dirname, '../premium.json'), (eventType, filename) => {
      console.log('premium.json update')
      resolve(json())
    })
  })
}

const premi = await dataNew()
...
const processMessage = async (m, commands, conn) => {
  for (const msg of m.messages) {
    ...
    const config = {
      ...
      quoted: {
        remoteJid: rep?.participant || 'not found',
        id: rep?.stanzaId || 'not found',
        fromMe: false
      }
    }
      ...
      if (cmd === 'menu') {
        const content = {
          ...
          contextInfo: {
            externalAdReply: {
              title: `*Hello* ${user}`,
              thumbnai: fs.readFileSync(join(__dirname, '../src/menu.jpg')),
              mediaType: 1,
