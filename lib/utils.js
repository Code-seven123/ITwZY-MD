...
async function encript(data, key = "1234") {
  return new Promise(resolve => {
    const cipher = Cipher("aes-256-cbc", key)
    let enc = cipher.update(typeof data == "object" ? JSON.stringify(data) : data)
    enc = Buffer.concat([enc, cipher.final()])
    resolve(enc.toString("base64"))
  })
}
async function decript(encript, key = "1234") {
  return new Promise(resolve => {
    const decipher = Decipher("aes-256-cbc", key)
    const data = Buffer.from(encript, "base64")
...
async function ArrayChunk(array, length) {
  return new Promise(resolve => {
    const chunks = [];
