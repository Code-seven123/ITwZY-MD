  ...
  for( const plugin of plugins ){
    const resultData = plugin.data.map(item => {
      ...
      for( const item of commandName ){
        styled.push({ header: `${item} ${args}`, title: `${prefix}${item} info` })
      }
      return styled
    }).flat(1)
    const rows = []
...
async function menu(user, storage, id){
  ...
  const contact = contactsArray.filter(i => i.id.endsWith("s.whatsapp.net")).length
  const group = Object.keys(storage.messages).filter(i => i.endsWith("g.us")).length
