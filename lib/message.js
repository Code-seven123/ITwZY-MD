...
import { limit, prefix, botName, messageLoading, setupAI, apiKey } from "./config.js"
...
const chatAi = aiModel.startChat({
  history: [
    {
      role: "user",
      parts: [{ text: setupAI }]
...
export default async (conn, m, storage) => {
  ...
  for (const msg of m.messages) {
    ...
    if (((status === "premium" && usePremium) || status === "admin")&& regex.test(data?.text)) {
      console.log("Hello testing sukses")
      resultAi = await chatAi.sendMessage(data?.text)
      await conn.sendMessage(data?.id, { text: resultAi?.response?.text() || "Tidak mendapatkan response" })
    } else if (prefix === editedData?.messagePrefix) {
      ...
      if (/^(menu)$/i.test(editedData?.commandName)) {
        ...
        await conn.sendMessage(data?.id, content, { quoted: msg })
      }
    ...
    if(data?.text != undefined || data?.username != "" || data?.username != undefined || data?.text !== null || data?.username != null) {
      m.type = m.type == "append" ? "from system" : "from member"
      console.log(
        "replying to",
        msg?.key?.remoteJid,
