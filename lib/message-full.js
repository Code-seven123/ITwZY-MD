...
export default async (conn, m, storage) => {
  for( const msg of m.messages ) {
    ...
    msg.interactiveMessage = async (jid, body, footer, title, subtitle, media, buttons, quoted, options = {}) => {
    ...
    let data = {
      ...
      text: msg?.message?.interactiveResponseMessage?.body?.text || msg?.message?.conversation || (msg?.message?.videoMessage || msg?.message?.imageMessage)?.caption || msg?.message?.extendedTextMessage?.text
    }
    const editedData = {
    ...
    if(prefix === editedData?.messagePrefix){
      ...
      if(/^(menu)$/i.test(editedData?.commandName)) {
        const { header, list } = await menu(data?.username, storage, id)
        const content = {
          text: header,
          contextInfo: {
            externalAdReply: {
        ...
        await conn.relayMessage(id, list.message, {
          messageId: list.key.id
        })
      }
      for( const command of commands ) {
        const now = new Date().getTime()
        if(global.dataPenalti[config.personalId]?.count >= limit && !(status === "admin") && !command?.noLimit) {
          ...
          const dataLimit = {
            [config?.personalId]: {
              count: global.dataPenalti[config.personalId]?.count === undefined ? 1 : global.dataPenalti[config.personalId]?.count + 1,
              timestamps: 1
            }
          }
